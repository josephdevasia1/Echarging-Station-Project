...
/**
 * Setting the product quantity to 1 always.
 */

add_action('save_post_product', 'auto_set_stock_to_one', 10, 3);

function auto_set_stock_to_one($post_id, $post, $update) {
    // Ensure we're saving a product
...
/**
 * Disabling shipping (Setting the products as virtual) for all products
 */

add_action('save_post_product', 'set_product_as_virtual', 10, 3);

function set_product_as_virtual($post_id, $post, $update) {
    if ('product' != $post->post_type) {
        return;
    }
